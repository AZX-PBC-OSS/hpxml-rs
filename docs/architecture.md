...
└── hpxml-core          ← glue: inspect, depth reader, parse/serialize wrappers
    ...
    ├── hpxml-types-v2  ← generated v2 types (~290K LOC), optional
    ├── hpxml-types-v3  ← generated v3 types (~430K LOC), optional
    ├── hpxml-types-v4  ← generated v4 types (~510K LOC), optional
    └── hpxml-types-v5  ← generated v5 types (~515K LOC), optional
```
...
Each version generates ~290-515K lines of Rust (v2 ~290K, v3 ~430K, v4 ~510K,
v5 ~515K; ~1.7M total). Rust's compilation unit is the crate.
With all versions in a single crate, ~1.7M LOC compiles serially. Splitting into
...
3. **Minimal glue**: handwritten code is inspect, config, errors, depth-limiting reader,
   and `impl_version_module!` plus one `conformance_tests!` invocation per version
4. **Adding a version = one macro invocation** plus a feature gate, a namespace
   entry, and a fixture directory — zero handwritten parser/serializer

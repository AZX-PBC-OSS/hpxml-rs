    ...
    ($mod_name:ident, feature = $feature:literal, version = $version:ident, fixture = $fixture:literal) => {
        ...
        mod $mod_name {
            ...
            fn parse_and_access_building_fields() {
                ...
                let b = doc.building[0].as_ref().unwrap();
            ...
            fn strip_optional_fields_roundtrip() {
                ...
                let reparsed = roundtrip(&doc);
                let b = reparsed.building[0].as_ref().unwrap();
...
mod v4_usage {
    ...
    fn parse_modify_serialize() {
        ...
        let second = {
            let mut b = doc.building[0].as_ref().unwrap().clone();
    ...
    fn nillable_mixed_nil_and_value_roundtrip() {
        ...
        assert!(!reparsed.building[2].is_nil());
        assert!(reparsed.building[3].is_nil());
        assert_eq!(
            reparsed.building[0].as_ref().unwrap().building_id.id,
            "First"
        );
        assert_eq!(
            reparsed.building[2].as_ref().unwrap().building_id.id,
            "Third"
        );
    }
    ...
    fn clone_and_modify_building_details() {
        ...
        let mut cloned = doc.building[0].as_ref().unwrap().clone();

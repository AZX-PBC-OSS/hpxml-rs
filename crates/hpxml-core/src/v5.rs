...
mod tests {
    ...
    fn test_parse_bpi2101_xml() {
        let xml = include_bytes!("../tests/data/v5/bpi2101.xml");
        let result = parse(xml);
    ...
    fn test_parse_upgrade_xml() {
        let xml = include_bytes!("../tests/data/v5/upgrade.xml");
        let result = parse(xml);
    ...
    fn test_invalid_utf8() {
        let xml = br#"<?xml version="1.0" encoding="UTF-8"?><HPXML xmlns="http://hpxmlonline.com/2025/12" schemaVersion="5.0"><Building><BuildingID id="a">test\xFF\xFE</BuildingID></Building></HPXML>"#;
        let result = parse(xml);
        assert!(result.is_err());
    }

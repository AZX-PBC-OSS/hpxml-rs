...
mod tests {
    ...
    fn test_parse_bpi2101_xml() {
        let xml = include_bytes!("../tests/data/v2/bpi2101.xml");
        let result = parse(xml);
    ...
    fn test_parse_upgrade_xml() {
        let xml = include_bytes!("../tests/data/v2/upgrade.xml");
        let result = parse(xml);
    ...
    fn test_invalid_utf8() {
        let xml = br#"<?xml version="1.0" encoding="UTF-8"?><HPXML xmlns="http://hpxmlonline.com/2014/6" schemaVersion="2.3"><Building><BuildingID id="a">test\xFF\xFE</BuildingID></Building></HPXML>"#;
        let result = parse(xml);
        assert!(result.is_err());
    }
